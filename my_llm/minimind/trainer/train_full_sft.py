...
def train_epoch(epoch, loader, iters, start_step=0, wandb=None):
    start_time = time.time() #记录本epoch的开始时间
    for step, (input_ids, labels) in enumerate(loader, start=start_step + 1):#遍历数据加载器，start_step 用于从断点续训时从特定 Step 开始
        ...
        with autocast_ctx:#混合精度下的前向传播
            ...
            loss = res.loss + res.aux_loss#计算总损失==预测损失(loss) + 负载均衡辅助损失(aux_loss）
            loss = loss / args.accumulation_steps#误差平均化

        scaler.scale(loss).backward()#在反向传播前先进行放大（防止精度不足被截断为0），再进行反向传播

        if (step + 1) % args.accumulation_steps == 0:#每当达到触发梯度累计更新的步数时
            scaler.unscale_(optimizer)#先缩放回原本大小
            torch.nn.utils.clip_grad_norm_(model.parameters(), args.grad_clip)#再对梯度进行裁剪，防止梯度爆炸

            scaler.step(optimizer) #更新模型参数
            scaler.update() #根据本次训练梯度情况更新下一次的缩放因子

            optimizer.zero_grad(set_to_none=True)  #清空梯度缓冲区，set_to_none能略微提升性能并节省显存
         #以下是日志打印、监控
...
if __name__ == "__main__":
    ...
    parser.add_argument('--from_weight', default='pretrain', type=str, help="基于哪个权重训练，为none则不基于任何权重训练")#与预训练的区别，在预训练权重基础上进行训练
